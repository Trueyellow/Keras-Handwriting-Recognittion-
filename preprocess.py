...
preprocess.py
This is a image preprocessing model that can help us deal with raw image image, and help us get much better perfomance
in training and real-life prediction.
"""
import cv2
import numpy as np
...
def line_extraction(picture_name):
    ...
    show_image = image[0:image.shape[1],0:image.shape[0]]
    ...
    for i in range(min(len(above_index), len(below_index))):
        if below_index[i] - above_index[i] > 20:
            ...
            # cv2.line(show_image, (0, above_index[i]), (show_image.shape[1], above_index[i]), (0, 0, 255), 3)
            # cv2.line(show_image, (0, below_index[i]), (show_image.shape[1], below_index[i]), (0, 0, 255), 3)
            # filename = "%d.jpg" % d
...
def extract_single_character(line_list):
    ...
    for image in line_list:
        gray1 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # grayscale
        _, thresh = cv2.threshold(gray1, 110, 255, cv2.THRESH_BINARY)  # threshold
        d += 1
        _, binary_erosion = cv2.threshold(gray1, 120, 255, cv2.THRESH_BINARY_INV)
        ...
        for i in range(len(sort_y_contour)):
            ...
            test = cut(test, 1)
            d = d + 1
            cv2.imwrite('C:\\Users\\hkx\\Desktop\\Imagepreprocessing\\cut_image{}.jpg'.format(d), test)
...
def write_text(picture_name, result_list):
    image = cv2.imread(picture_name)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    for i in range(min(len(above_index), len(below_index))):
        if below_index[i] - above_index[i] > 20:
            ...
            cv2.line(show_image, (0, above_index[i]), (show_image.shape[1], above_index[i]), (0, 0, 255), 3)
            cv2.line(show_image, (0, below_index[i]), (show_image.shape[1], below_index[i]), (0, 0, 255), 3)
            cv2.imwrite("C:\\Users\\hkx\\Desktop\\Imagepreprocessing\\line_image{}.jpg".format(d), newImage)
            cv2.putText(show_image, result_list[d], (0, below_index[i] + 80), 1, 6, (255, 0, 0), 1)
            # filename = "%d.jpg" % d
            d = d + 1
            line.append(newImage)
        else:
    ...
    cv2.imwrite("C:\\Users\\hkx\\Desktop\\show\\image.jpg", show_image)
    im = Image.open("C:\\Users\\hkx\\Desktop\\show\\image.jpg")
    im.show()